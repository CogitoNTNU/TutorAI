...
from flashcards.embeddings import cosine_similarity
...
class DatabaseInterface(ABC):
    ...
    def __subclasscheck__(cls, subclass: any) -> bool:
        return (
            ...
            hasattr(subclass, 'post_curriculum') and
            callable(subclass.post_curriculum)
        )
...
class Database(DatabaseInterface):
    ...
    def get_curriculum(self, embedding: list[float]) -> list[str]:
        ...
        for document in documents:
            if cosine_similarity(embedding, document['embedding']) < self.similarity_threshold:
