...
class MongoDBTestCase(TestCase):
    ...
    def test_get_curriculum(self):
        ...
        curriculum = self.mongo.get_curriculum(OpenAIEmbedding().get_embedding("Den romersk-katolske kirken var stor i Texas under revolusjonen. Sam Houston var personlig en stor tilhenger"))
        self.assertEqual(len(curriculum), 1)
    ...
    def test_delete_pdf_pages(self):
        ...
        curriculum = self.mongo.get_curriculum(OpenAIEmbedding().get_embedding(self.curriculum["pdf1"]))
        self.assertEqual(len(curriculum), 0)
